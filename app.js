...
function setupRecognition() {
  ...
  recognition.onerror = (e) => {
    ...
    // Altres errors es resolen sols al onend
  };
  ...
  recognition.onend = () => {
    ...
    // En mode conversa: enviar si hi ha contingut, o re-escoltar si no
    if (conversationMode) {
      if (text && userIsSpeaking) {
        input.value = text;
        adjustInputHeight();
        sendMessage(text);
        // Després de sendMessage es tornarà a escoltar automàticament des de runConversationLoop
      } else if (!pendingRestart) {
        // Buit: ha estat un timeout natural de Safari. Reiniciar.
        pendingRestart = true;
        setTimeout(() => {
          pendingRestart = false;
          if (conversationMode && !isSending && !isListening) {
            startListening();
          }
        }, RESTART_DELAY_MS);
      }
    } else {
      // Mode toc puntual: enviar si hi ha text
      if (text) {
        input.value = text;
        adjustInputHeight();
        sendMessage(text);
...
function startListening() {
  ...
  // Si el reconeixement està en mal estat, recreem-lo
  if (!recognition) setupRecognition();
  try {
    recognition.start();
  } catch (e) {
    // Si peta (típicament perquè encara està actiu), recreem i provem un cop
    setTimeout(() => {
      setupRecognition();
