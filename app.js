...
let activeMode = null;  // null | 'tarea' | 'idea' | 'busqueda' | 'charla'
...
async function forceSaveState() {
  ...
  try {
    ...
    hasPendingChanges = true;  // Marcarem per reintentar després
  }
}
...
function renderMessages() {
  ...
  // Trobar quins missatges de "Pendiente" han estat resolts posteriorment
  // Un Pendiente queda resolt si DESPRÉS hi ha un 'ok' (creació realitzada) abans d'un altre 'warn'
  const resolvedPendings = new Set();
  for (let i = 0; i < state.messages.length; i++) {
    const m = state.messages[i];
    if (m.role === 'system' && m.kind === 'warn') {
      for (let j = i+1; j < state.messages.length; j++) {
        const n = state.messages[j];
        if (n.role === 'system' && n.kind === 'ok') {
          resolvedPendings.add(i);
          break;
        }
        // Si troba un altre warn nou, no comptem (un warn nou no resol l'anterior)
        if (n.role === 'system' && n.kind === 'warn') break;
      }
    }
  }
  ...
  state.messages.forEach((m, idx) => {
    ...
    if (m.role === 'system') {
      // Marcadors compactes
      if (m.kind === 'ok') {
        html = `<div class="bubble"><span class="status-mark ok">✓ ${escapeHtml(m.content)}</span></div>`;
      } else if (m.kind === 'warn') {
        const cls = resolvedPendings.has(idx) ? 'status-mark warn resolved' : 'status-mark warn';
        html = `<div class="bubble"><span class="${cls}">⚠ ${escapeHtml(m.content)}</span></div>`;
      } else {
        // Compatibilitat amb missatges antics
        html = `<div class="bubble">${escapeHtml(m.content)}</div>`;
      }
    } else {
      html = `<div class="bubble">${escapeHtml(m.content)}`;
      if (m.toolTrace) {
...
function addTask(task) {
  ...
  forceSaveState();  // Guardat immediat, sense debounce
  updateTasksBadge();
  renderTasks();
  return newTask;
}
...
const tools = [
  ...
  {
    name: 'calcular_fecha',
    description: 'Calcula una fecha en formato YYYY-MM-DD a partir de una expresión en lenguaje natural. SIEMPRE usa esta tool para calcular fechas relativas (mañana, el miércoles que viene, en 2 semanas, el día 20, etc.) ANTES de crear o modificar una tarea con fecha. NUNCA calcules fechas mentalmente, siempre usa esta herramienta porque las matemáticas de fechas son fiables solo si las hace el sistema.',
    input_schema: {
      type: 'object',
      properties: {
        expresion: { type: 'string', description: 'Expresión en lenguaje natural: "mañana", "el miércoles que viene", "en 2 semanas", "el 20", "el viernes", "pasado mañana", etc.' }
      },
      required: ['expresion']
    }
  }
...
function executeTool(name, input) {
  if (name === 'crear_tasca') {
    ...
    // Verificació real: la tasca està al state?
    const verificada = state.tasks[task.category].find(t => t.id === task.id);
    if (!verificada) {
      return {
        success: false,
        error: 'CRÍTICO: la tarea no se ha guardado correctamente. NO digas a Josep Maria que está apuntada. Vuelve a intentarlo o avisa del error.'
      };
    }
    return {
      success: true,
      id: task.id,
      tarea_creada: { text: task.text, category: task.category, deadline: task.deadline, priority: task.priority },
      total_tareas_ahora: { pro: state.tasks.pro.length, personal: state.tasks.personal.length },
      missatge: `Tarea apuntada correctamente en ${task.category === 'pro' ? 'Profesional' : 'Personal'}. Ya puedes confirmar a Josep Maria que está hecho.`
    };
  }
  ...
  if (name === 'calcular_fecha') {
    return calcularFechaTool(input.expresion || '');
  }
  return { error: 'Tool desconegut' };
}

// ---------- CALCULAR FECHA (deterministico) ----------
function calcularFechaTool(expr) {
  ...
  const diaSetmana = ['domingo','lunes','martes','miércoles','miercoles','jueves','viernes','sábado','sabado'];
  // Mapeo a getDay() (0=domingo)
  const diaIdx = { 'domingo':0,'lunes':1,'martes':2,'miércoles':3,'miercoles':3,'jueves':4,'viernes':5,'sábado':6,'sabado':6 };

  const fmtResp = (d, etiqueta) => ({
    fecha: formato(d),
    dia_semana: ['domingo','lunes','martes','miércoles','jueves','viernes','sábado'][d.getDay()],
    descripcion: etiqueta || `${['domingo','lunes','martes','miércoles','jueves','viernes','sábado'][d.getDay()]} ${d.getDate()} de ${MESOS[d.getMonth()]} ${d.getFullYear()}`
  });

  // Hoy
  if (/^(hoy|hoi)\b/.test(e)) return fmtResp(hoy);

  // Mañana
  if (/^(mañana|maniana|demà|dema)\b/.test(e) && !/pasado/.test(e)) {
    const d = new Date(hoy); d.setDate(d.getDate()+1); return fmtResp(d);
  }

  // Pasado mañana
  if (/pasado mañana|pasado maniana/.test(e)) {
    const d = new Date(hoy); d.setDate(d.getDate()+2); return fmtResp(d);
  }
  ...
  let m = e.match(/en (\d+) d[ií]as?/);
  if (m) { const d = new Date(hoy); d.setDate(d.getDate()+parseInt(m[1])); return fmtResp(d); }
  m = e.match(/en (\d+) semanas?/);
  if (m) { const d = new Date(hoy); d.setDate(d.getDate()+parseInt(m[1])*7); return fmtResp(d); }
  m = e.match(/en (\d+) mes(es)?/);
  if (m) { const d = new Date(hoy); d.setMonth(d.getMonth()+parseInt(m[1])); return fmtResp(d); }

  // "el día 20" o "el 20" (día del mes actual o siguiente si ya pasó)
  m = e.match(/(?:el (?:d[ií]a )?)?(\d{1,2})(?:\s*(?:de|\/)?\s*([a-záéíóúñ]+))?/);
  if (m) {
    const diaMes = parseInt(m[1]);
    const mesStr = m[2];
    if (diaMes >= 1 && diaMes <= 31) {
      let mesIdx = hoy.getMonth();
      let anyo = hoy.getFullYear();
      if (mesStr) {
        const idxMes = MESOS.findIndex(x => x.startsWith(mesStr.slice(0,3)));
        if (idxMes >= 0) {
          mesIdx = idxMes;
          if (mesIdx < hoy.getMonth() || (mesIdx === hoy.getMonth() && diaMes < hoy.getDate())) {
            anyo++;
          }
        }
      } else {
        // Si no especifica mes, si el día ya pasó este mes -> mes siguiente
        if (diaMes < hoy.getDate()) mesIdx++;
        if (mesIdx > 11) { mesIdx = 0; anyo++; }
      }
      const d = new Date(anyo, mesIdx, diaMes);
      return fmtResp(d);
    }
  }

  // "el miércoles", "el miércoles que viene", "el próximo miércoles"
  for (const [nombre, idx] of Object.entries(diaIdx)) {
    if (e.includes(nombre)) {
      const hoyDia = hoy.getDay();
      let diff = idx - hoyDia;
      if (diff <= 0) diff += 7; // Si es hoy o ya pasó esta semana, va a la próxima
      if (/que viene|próximo|proximo|siguiente/.test(e) && diff < 7) {
        // ya está en próxima semana por defecto
      }
  ...
  // No reconocido
  return { error: `No he sabido calcular la fecha de "${expr}". Pídele a Josep Maria que te la diga en formato YYYY-MM-DD o más concreta.` };
}
...
function buildSystemPrompt(mode) {
  ...
  // Obtenir categories existents per ajudar a reutilitzar-les
  const catTasks = new Set();
  state.tasks.pro.forEach(t => t.text && catTasks.add('pro'));
  state.tasks.personal.forEach(t => t.text && catTasks.add('personal'));
  const catIdeas = [...new Set((state.ideas || []).map(i => i.category))];
  const catPerfil = [...new Set((state.perfil || []).map(e => e.category))];
...
⚡ MODO FORZADO: TAREA
Josep Maria ha activado el modo TAREA antes de este mensaje. El contenido que te ha enviado es una tarea — NO lo cuestiones, NO preguntes si es tarea o idea, simplemente créala con 'crear_tasca'. Si menciona una fecha relativa (mañana, el miércoles, etc.), usa SIEMPRE 'calcular_fecha' antes. Lo único que puedes preguntar (y solo si NO está claro) es: la prioridad, si tiene fecha límite, o si es profesional/personal. Si todo está claro o es deducible, créala sin preguntar nada y confirma brevemente.`;
  } else if (mode === 'idea') {
    modeInstruction = `

⚡ MODO FORZADO: IDEA
Josep Maria ha activado el modo IDEA antes de este mensaje. El contenido es algo que quiere guardar como idea — NO lo cuestiones. Llama a 'guardar_idea' con un título y descripción adecuados. Para la categoría: revisa las que ya existen (${catIdeas.length > 0 ? catIdeas.join(', ') : 'ninguna todavía'}) y reutiliza una si encaja. Si crees que ninguna encaja, propón una nueva preguntándole brevemente: "¿Lo guardo en [categoría existente] o creo una nueva 'X'?"`;
  } else if (mode === 'busqueda') {
    modeInstruction = `

⚡ MODO FORZADO: BÚSQUEDA
Josep Maria quiere que busques información. Por ahora aún no tienes herramienta de búsqueda web disponible — dile que esto se activará pronto y, mientras, intenta responder con lo que sepas o ayúdale a refinar la pregunta para cuando esté lista la búsqueda.`;
  } else if (mode === 'charla') {
    modeInstruction = `

⚡ MODO FORZADO: CHARLA
Josep Maria quiere conversar sin que guardes nada. NO uses 'crear_tasca', NO uses 'guardar_idea', NO uses 'afegir_al_perfil'. Solo conversa: debate, da opinión, pregunta. Aunque mencione cosas que parezcan tareas o ideas, NO las guardes en este mensaje.`;
  }

  return `Eres Claudie, la asistente personal de Josep Maria Jauma.${modeInstruction}
...
1. **GESTIÓN DE TAREAS — REGLA DE ORO**: si Josep Maria menciona CUALQUIER acción que debe hacer, aunque no diga explícitamente "apúntalo", debes apuntarla. Frases como "mañana tengo que llamar a X", "tengo que comprar Y", "no olvidar Z", "recuérdame X" → TODAS son órdenes de apuntar tarea. NO te limites a confirmar con palabras: usa la tool 'crear_tasca' SIEMPRE.

**REGLA CRÍTICA — FECHAS**: NUNCA calcules una fecha mentalmente. Si Josep Maria dice "mañana", "el miércoles", "el día 20", "la semana que viene", etc., USA SIEMPRE la herramienta 'calcular_fecha' PRIMERO para obtener la fecha exacta en formato YYYY-MM-DD, y solo después crea o modifica la tarea con esa fecha. Equivocarse en una fecha es un error grave de confianza.

**REGLA CRÍTICA — NO MENTIR**: NUNCA digas "apuntada", "anotada", "te lo guardo", "lo apunto", "ya está", "ya la tienes" o similar SIN haber llamado realmente a la herramienta 'crear_tasca' EN EL MISMO MENSAJE de respuesta. Tu respuesta de texto y tu llamada a la tool deben ir SIEMPRE juntas en el mismo turno. Si has hecho una modificación a otra tarea pero no creas una nueva, NO digas que has apuntado la nueva. Si te das cuenta de que se te ha "olvidado" llamar a la tool, en ese mismo turno llámala — no en el siguiente.

REVISIÓN AUTOMÁTICA OBLIGATORIA: antes de redactar tu mensaje de texto, comprueba mentalmente: "¿Estoy a punto de decir que he apuntado algo? Si sí, ¿he llamado a crear_tasca en este turno? Si no, llámala AHORA antes de responder."

Si Josep Maria te da varias tareas en un solo mensaje (ej. "apunta esto, esto y esto"), llama a 'crear_tasca' TANTAS VECES como tareas haya, todas en la misma respuesta.

Solo excepción para no apuntar: si está hablando en hipotético ("si fuera por mí haría X") o reflexionando en voz alta sin compromiso ("quizá debería..."). Ante la duda, apúntala. También tienes herramientas 'llistar_tasques', 'modificar_tasca', 'marcar_tasca' y 'eliminar_tasca'. No digas nunca que "no puedes" hacerlo.
...
async function sendMessage(text) {
  ...
  // Capturar el mode actiu i desactivar-lo
  ...
  state.messages.push({ role: 'user', content: text, ts: Date.now(), mode: modeForThisMessage });
  ...
  try {
    await runConversationLoop(modeForThisMessage);
  } catch (e) {
    hideTyping();
    state.messages.push({
      role: 'assistant',
...
async function runConversationLoop(mode) {
  // Bucle: pots fer fins a 5 rondes de tool_use abans de tornar la resposta final
  const maxRounds = 5;
  let anyToolCalled = false;
  let createdInThisTurn = false;  // S'ha cridat alguna eina de creació en aquest torn?
  ...
  for (let round = 0; round < maxRounds; round++) {
    ...
    if (stopReason !== 'tool_use' || toolUseBlocks.length === 0) {
      ...
      if (finalText) {
        ...
        // Detector: si Claudie diu que ha CREAT alguna cosa però no ha cridat cap tool de creació en aquest torn
        const claimsCreation = /\b(apunt|anot|guard|registr|cread|añadid|memoriz)/i.test(finalText);
        if (claimsCreation && !createdInThisTurn) {
          state.messages.push({
            role: 'system',
            kind: 'warn',
            content: 'Pendiente',
    ...
    anyToolCalled = true;
    // Marcar si en aquest torn s'ha cridat alguna eina de creació
    if (toolUseBlocks.some(t => ['crear_tasca','guardar_idea','afegir_al_perfil'].includes(t.name))) {
      createdInThisTurn = true;
    }

    // Executar les tools
    const toolResults = [];
    const toolActions = [];
    const toolActionsWarn = [];
    for (const tu of toolUseBlocks) {
      ...
      // Generar etiqueta visible per a l'usuari (només la paraula clau)
      if (tu.name === 'crear_tasca') {
        if (result.success) toolActions.push('Apuntada');
        else toolActionsWarn.push('Error al apuntar');
      } else if (tu.name === 'modificar_tasca' && result.success) {
        toolActions.push('Modificada');
      } else if (tu.name === 'marcar_tasca' && result.success) {
        toolActions.push(result.missatge === 'Marcada com a feta' ? 'Hecha' : 'Pendiente');
      } else if (tu.name === 'eliminar_tasca' && result.success) {
        toolActions.push('Eliminada');
      } else if (tu.name === 'afegir_al_perfil' && result.success) {
        toolActions.push('Anotado en perfil');
      } else if (tu.name === 'eliminar_del_perfil' && result.success) {
        toolActions.push('Eliminada del perfil');
      } else if (tu.name === 'guardar_idea') {
        if (result.success) toolActions.push('Idea guardada');
        else toolActionsWarn.push('Error al guardar idea');
      } else if (tu.name === 'modificar_idea' && result.success) {
        if (tu.input.status === 'descartada') toolActions.push('Idea descartada');
        else if (tu.input.status === 'proyecto') toolActions.push('Idea → proyecto');
    ...
    if (toolActionsWarn.length > 0) {
      state.messages.push({
        role: 'system',
        kind: 'warn',
        content: toolActionsWarn.join(' · '),
        ts: Date.now()
      });
      renderMessages();
    }
...
function bindAppEvents() {
  ...
  document.querySelectorAll('.mode-chip').forEach(chip => {
    chip.addEventListener('click', () => {
      ...
      if (activeMode === mode) {
        // Toggle: desactivar si ja estava actiu
        activeMode = null;
      } else {
        activeMode = mode;
      }
