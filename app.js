...
function renderMessages() {
  ...
  // (un tool call de creació o modificació posterior els resol)
  const resolvedPendings = new Set();
  for (let i = 0; i < state.messages.length; i++) {
    const m = state.messages[i];
    if (m.role === 'system' && m.kind === 'warn') {
      // Buscar si després hi ha algun missatge de sistema 'ok' (acció realitzada)
      for (let j = i+1; j < state.messages.length; j++) {
        ...
        // Si troba un missatge d'usuari nou abans de 'ok', no comptem
  ...
  for (let round = 0; round < maxRounds; round++) {
    ...
    for (const tu of toolUseBlocks) {
      ...
      if (tu.name === 'crear_tasca' && result.success) {
        ...
        toolActions.push('Idea guardada');
