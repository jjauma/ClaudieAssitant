...
const VOICE_HEALTH_MS = 8000;       // sense rebre senyal en X temps, reiniciem
...
let healthTimer = null;            // detecta si Safari s'ha quedat "mut"
let lastEventTs = 0;               // timestamp últim event onresult
let lastTranscript = '';
let finalTranscript = '';
...
function setupRecognition() {
  ...
  recognition.onresult = (event) => {
    ...
    if (current !== lastTranscript) {
      ...
      resetSilenceTimer();
      resetInactivityTimer();
      // Flash visual: el botó s'il·lumina més fort quan rep veu
      document.getElementById('mic-btn').classList.add('hearing');
      setTimeout(() => document.getElementById('mic-btn').classList.remove('hearing'), 200);
    }
  };
  ...
  recognition.onerror = (e) => {
    if (e.error === 'not-allowed') {
      ...
      // Errors transitoris, no fer res - el onend s'encarregarà
    } else {
  ...
  recognition.onend = () => {
    ...
    if (conversationMode) {
      if (text && userIsSpeaking) {
        ...
        adjustInputHeight();
        sendMessage(text);
        // Recreem el reconeixedor net pel següent torn
        recreateRecognition();
      } else {
        // Sense contingut: probablement timeout, reiniciem el reconeixedor
        recreateRecognition();
        setTimeout(() => {
...
function startHealthCheck() {
  ...
  healthTimer = setInterval(() => {
    ...
    const sinceLastEvent = Date.now() - lastEventTs;
    if (sinceLastEvent > VOICE_HEALTH_MS) {
      // Safari està "mut", reiniciem
      console.log('Safari mut detectat, reiniciant reconeixement');
      const status = document.getElementById('mic-status');
      status.textContent = 'Reiniciant micro...';
      status.classList.add('visible');
      setTimeout(() => status.classList.remove('visible'), 1500);
      try { recognition.stop(); } catch (e) {}
      // onend s'ocupa de recrear i tornar a escoltar
    }
  }, 2000);
}
